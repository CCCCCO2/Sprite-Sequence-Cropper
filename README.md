...
一个 Photoshop（PS） 脚本，主要解决游戏开发中 2D 序列帧图片的裁剪、计算锚点偏移等批量处理需求。脚本提供手动框选、针对人类或类人形生物的自动裁剪，以及所有帧使用同一水平锚点的统一锚点裁剪，可以在保持锚点居中的同时导出相邻序列帧之间的锚点偏移。该数据可供游戏引擎（Unity/UE）中的 Sprite 使用，以实现更精确的动画位移并保持原帧动画表现。

由于是在游戏引擎中使用的 sprite，所以图片拥有并且要求有以下特点：

- .png 格式
- 图像模式为“RGB颜色”
- 裁剪边界以当前活动图层的非透明像素为准；如画面由多个图层组成，请先合并图层，或确保活动图层包含完整角色内容
- 对批量覆盖保存的要求是文档已经保存为目标 PNG 文件
...
- `首帧中心锚点`：要指定的中心锚点横坐标，支持非负整数和半像素坐标，例如 `92`、`92.5`。半像素锚点适用于中心位于两个像素之间的偶数宽区域，不接受其它任意小数，以保证对称裁剪边界落在整数像素坐标上.
- `启用指定尾帧锚点`：启用后序列帧动画的最后一帧裁剪将忽略“自动裁剪”或者“手动框选选区裁剪”，直接以指定的锚点为中心扩展裁剪选区。适用于需要衔接下一个动画的第一帧，对齐位置的情况.
- `尾帧中心锚点`：要指定的中心锚点横坐标，支持非负整数和半像素坐标，例如 `92`、`92.5`。半像素锚点适用于中心位于两个像素之间的偶数宽区域，不接受其它任意小数，以保证对称裁剪边界落在整数像素坐标上.
...
- `统一锚点裁剪`：所有打开的序列帧都使用同一个水平锚点 X 对称裁剪。每帧的纵向范围仍根据该帧活动图层的内容边界确定；该模式会忽略单独设置的首帧和尾帧锚点

**自动裁剪设置**

- `居中锚点方式`：指裁剪后图片锚点的居中方式，包括“底部中心”和“顶部中心”，“底部中心”为锚点保持在角色脚部中心，“顶部中心”为锚点保持在角色头顶中心
- `底部/顶部 范围`：用于识别角色头部或脚部中心的纵向像素范围。范围以活动图层实际内容的顶部/底部为基准，不受画布透明边距影响，且必须是正整数
...
- `所有帧中心锚点 X`：应用于全部打开文档的原图水平坐标，支持非负整数和 `0.5` 步进。脚本会以该 X 坐标为中心向左右对称扩展，直到包含活动图层的全部内容
...
- `前进方向为左`：角色前进方向是否朝左，不勾选代表朝右。该选项只会调整 CSV 中水平偏移量 X 的正负，使正数始终表示向角色前方移动；垂直偏移量 Y 不受左右朝向影响
...
启用“启用导出CSV”时，会在选择的文件夹路径下生成 UTF-8 编码的 `offsets.csv` 文件。每次裁剪会覆盖同名文件，避免混入上一次批处理的数据：
...
- `偏移量Y`：裁剪后图片的中心锚点在原图中的纵坐标相对于上一张图片的偏移量。在 Photoshop 左上角为原点的坐标系中，正数表示向下移动，负数表示向上移动；该值不受“前进方向为左”开关影响
